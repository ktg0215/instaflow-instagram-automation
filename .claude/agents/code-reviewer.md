---
name: code-reviewer
description: Use this agent when you have written, modified, or committed code and need a comprehensive review for quality, security, and maintainability. This agent should be used proactively after completing any logical chunk of development work, especially when configuration files have been changed. Examples: <example>Context: User has just implemented a new database connection pool configuration. user: 'I just updated our database connection settings to handle more concurrent users' assistant: 'Let me use the code-reviewer agent to examine these configuration changes for potential issues' <commentary>Since configuration changes are high-risk and can cause outages, use the code-reviewer agent to thoroughly analyze the changes with special focus on connection pool settings and their production impact.</commentary></example> <example>Context: User has completed a new feature implementation. user: 'I finished implementing the user authentication system with JWT tokens' assistant: 'Now let me use the code-reviewer agent to review the authentication implementation for security and best practices' <commentary>Since authentication code involves security considerations, use the code-reviewer agent to ensure proper security practices, error handling, and potential vulnerabilities are addressed.</commentary></example>
model: sonnet
---
